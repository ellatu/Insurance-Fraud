...
# 初始化孤立森林模型
iso_forest = IsolationForest(n_estimators=100, contamination=0.27, random_state=42)  # contamination参数表示异常点的比例
...
# 假设目标变量是 'fraud'
y_true = data_cleaned['fraud_reported']
...
# 查看异常点中欺诈样本的比例
fraud_in_anomalies = data_cleaned[data_cleaned['anomaly'] == 1]['fraud_reported'].mean()
print(f'Fraud Rate in Anomalies: {fraud_in_anomalies:.2f}')

# 尝试不同的contamination值
iso_forest_tuned = IsolationForest(n_estimators=100, contamination=0.3, random_state=42)  # 假设异常点比例为10%
