...
def plt_hist(data, bins, xlim=(0,600),
             ...
             title='D3 VAE Reconstruction Errors', show_threshold=True):
    # 计算KDE曲线
    # 保证数据非负
...
if __name__ == "__main__":
    ...
    #画出重构误差图
    #print(plt_hist(vae_errors, 50000))
    ...
    vae_errors_fraud['fraud'] = y_test1['FRAUDE']
    vae_errors_fraud = vae_errors_fraud[vae_errors_fraud['fraud'] == 1]
    vae_errors_fraud = vae_errors_fraud['are'].to_numpy()
    #画出欺诈与全部预测的对比
    #print(plt_hist2(vae_errors,vae_errors_fraud,50000,1000))

    # 设定阈值
    threshold = np.percentile(vae_errors, 80)
    high_error_indices = np.where(vae_errors > threshold)[0]
    vae_high_error_nl = np.mean(np.square(X_test[high_error_indices] - X_pred[high_error_indices]), axis=0)
    high_nl_re = pd.DataFrame({
        'feature': original_columns,  # 列名
        'recon_error': vae_high_error_nl  # 每列的重构误差
    })
    # 按值升序排序（从低到高）
    sorted_indices = np.argsort(vae_high_error_nl)[::-1]
    sorted_labels = [original_columns[i] for i in sorted_indices]
    sorted_values = [vae_high_error_nl[i] for i in sorted_indices]
    ...
    # 设定阈值
    threshold = np.percentile(vae_errors, 20)
    low_error_indices = np.where(vae_errors < threshold)[0]
    vae_low_error_nl = np.mean(np.square(X_test[low_error_indices] - X_pred[low_error_indices]), axis=0)
    # 将重构误差数组转换为带列名的Series或DataFrame
    low_nl_re = pd.DataFrame({
        'feature': original_columns,  # 列名
        'recon_error': vae_low_error_nl  # 每列的重构误差
    })
    # 按值升序排序（从低到高）
    sorted_indices = np.argsort(vae_low_error_nl)[::-1]
    sorted_labels = [original_columns[i] for i in sorted_indices]
    sorted_values = [vae_low_error_nl[i] for i in sorted_indices]
    top30_labels_desc = sorted_labels[:30][::-1]
    top30_values_desc = sorted_values[:30][::-1]
