...
def plt_hist(data, bins, xlim=(0,250),
             ...
             title='D2 AE Reconstruction Errors', show_threshold=True):
    # 计算并KDE曲线
    # 保证数据非负（根据你的数据调整偏移量）
...
if __name__ == "__main__":
    ...
    # 按值升序排序（从低到高）
    sorted_indices = np.argsort(ae_high_error_nl)
    ...
    # 设定阈值
    threshold = np.percentile(ae_errors, 20)
    ...
    # 按值升序排序（从低到高）
    sorted_indices = np.argsort(ae_low_error_nl)
    sorted_labels = [original_columns[i] for i in sorted_indices]
    ...
    #np.savetxt(output_path+"文件名", ae_errors, delimiter=',')  # 保存为CSV，修改