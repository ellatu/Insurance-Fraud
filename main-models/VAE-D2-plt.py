...
#单个图绘画函数
def plt_hist(data, bins, xlim=(0,250),
             hist_color='#006699', kde_color='blue',
             title='D2 VAE Reconstruction Errors', show_threshold=True):
...
#对比图绘画函数
def plt_hist2(data1, data2,bins1, bins2,xlim=(0,250),
             hist_color='#006699',
             title='D2 VAE Reconstruction Errors', show_threshold=True):
...
class VAE(Model):
    def __init__(self, input_dim, latent_dim=32,**kwargs):
        ...
        self.encoder = tf.keras.Sequential([
            layers.Dense(512, activation='tanh', dtype=tf.float32),
            layers.Dropout(0.2),
...
if __name__ == "__main__":
    # 清理数据
    X_scaled, y, X_train, X_test, y_train, y_test ,original_columns = clean_fuction('creditcard.csv', 'Class','Time')
    # 加载模型
    loaded_model = load_model(
        "vae_model_d2_5.keras",
    ...
    print(high_nl_re)
    #按值升序排序（从低到高）
    sorted_indices = np.argsort(ae_high_error_nl)  #获取排序后的索引
    sorted_labels = [original_columns[i] for i in sorted_indices]
    sorted_values = [ae_high_error_nl[i] for i in sorted_indices]

    #VAE变量重要性
    plt.figure(figsize=(10, 6))
    plt.bar(sorted_labels, sorted_values,color='#006699')
    plt.xlabel("Feature Name")
    plt.ylabel("Reconstruction Error")
    plt.title("D2 Feature Importance(upper-tail)")
    ...
    print(low_nl_re)
    #按值升序排序（从低到高）
    sorted_indices = np.argsort(ae_low_error_nl)  # 获取排序后的索引[1](@ref)
    sorted_labels = [original_columns[i] for i in sorted_indices]
    ...
    plt.xlabel("Feature Name")
    plt.ylabel("Reconstruction Error")
    plt.title("D2 Feature Importance(lower-tail)")
    plt.xticks(rotation=90)
    plt.show()
